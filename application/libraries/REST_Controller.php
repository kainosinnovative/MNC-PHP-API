...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        foreach (getallheaders() as $name => $value) {
            if ($value == "MNCLOTSOFLOVE") {
                $is_agent_auth = true;
            }
        }
        //        if(!$is_agent_auth) { die('unautherized');}
        // init objects
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (!method_exists($this, $controller_method)) {
            //sendErrorMail(array("Method" => $controller_method), "API | Someone is hitting wrong method");
            $this->response('', 404, 'fail', 'Unknown Method');
        }
     ...
     * @param int $value
     * @param string $param
     * @return string value if success
     */
    public function checkEmptyParam($value, $param)
    {
     ...
     */
    public function response($data = null, $http_code = null, $status = 'pass', $message = '')
    {
        global $CFG;
